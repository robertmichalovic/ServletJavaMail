...
@WebServlet("/OdeslaniHTMLMailu")
public class OdeslaniHTMLMailu extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private MailNastaveni mail;
    public OdeslaniHTMLMailu() {
        super();	}
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("Metoda Post");	
		response.setContentType("text/html");
		PrintWriter out = response.getWriter();
		mail = new MailNastaveni(request,"bezPrilohy");
		mail.vytvoreniMailuBezPrilohy();
		try{
			Transport.send(mail.getMail());
			String title = "Posilani HTML Mailu";
